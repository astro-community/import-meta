...
export class Assets extends Map {
	...
	add(path) {
		const pathname = posix.normalize(path)
...
export class Asset {
	/** @this {import('./asset').Asset} @arg {string} pathname */
	constructor(pathname) {
		...
		this.hashname = this.basename + hash(this.basename) + '.' + this.extension
		this.filetype = type(this.extension)
	}
}

/** Returns the file type from the given extension. */
export const type = (/** @type {string} */ extension) => typeByExtension[extension] || 'application/octet-stream'

/** Returns the hash for the given value (sha256 as the first 8 hex chars). */
export const hash = (/** @type {string} */ value) => crypto.createHash('sha256').update(value).digest('hex').slice(0, 8)
...
const $$symbol = Symbol.for('import.meta')
